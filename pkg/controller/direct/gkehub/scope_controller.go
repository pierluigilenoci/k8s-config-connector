...
func (a *gkeHubScopeAdapter) Find(ctx context.Context) (bool, error) {
	...
	actual, err := a.hubClient.v1.Projects.Locations.Scopes.Get(a.id.String()).Context(ctx).Do()
	if err != nil {
...
func (a *gkeHubScopeAdapter) Delete(ctx context.Context, deleteOp *directbase.DeleteOperation) (bool, error) {
	op, err := a.hubClient.v1.Projects.Locations.Scopes.Delete(a.id.String()).Context(ctx).Do()
	if err != nil {
		if direct.IsNotFound(err) {
			return false, nil
		}
...
func (a *gkeHubScopeAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	parent := a.id.Parent()
	op, err := a.hubClient.v1.Projects.Locations.Scopes.Create(parent, desired).ScopeId(a.id.ID()).Context(ctx).Do()
	if err != nil {
...
func (a *gkeHubScopeAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	...
	if !reflect.DeepEqual(a.actual.NamespaceLabels, desired.NamespaceLabels) {
		...
		op, err := a.hubClient.v1.Projects.Locations.Scopes.Patch(a.id.String(), desired).UpdateMask(updateMask).Context(ctx).Do()
		if err != nil {
...
func (a *gkeHubScopeAdapter) waitForOp(ctx context.Context, op *gkehubapi.Operation) error {
	...
	for {
		current, err := a.hubClient.v1.Projects.Locations.Operations.Get(op.Name).Context(ctx).Do()
		if err != nil {
