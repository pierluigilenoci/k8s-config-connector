...
func (a *gkeHubNamespaceAdapter) Create(ctx context.Context, createOp *directbase.CreateOperation) error {
	...
	// Update status
	status := GKEHubNamespaceStatus_FromProto(mapCtx, a.actual)
	if mapCtx.Err() != nil {
		return mapCtx.Err()
	}
...
func (a *gkeHubNamespaceAdapter) Update(ctx context.Context, updateOp *directbase.UpdateOperation) error {
	log := klog.FromContext(ctx)
	log.V(2).Info("updating GKEHubNamespace", "id", a.id.String())
	...
	mapCtx := &direct.MapContext{}
	// GKEHubNamespace currently has no mutable fields in its spec.
...
func (a *gkeHubNamespaceAdapter) waitForOp(ctx context.Context, op *gkehubapi.Operation) error {
